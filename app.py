...
if not st.session_state.authenticated:
    ...
    input_password = st.text_input(
        "Գաղտնաբառ", type="password", placeholder="Գրիր պասվորդը..."
    )
...
# --- 2. 📦 ԱՊՐԱՆՔԻ ՁԵՌՔԲԵՐՈՒՄ ---
    ...
    with col2:
        ...
        nshumner = st.text_input(
            "📌 Լրացուցիչ Նշումներ", placeholder="Նշումներ..."
        )
    ...
    if image:
        st.info(
            "📷 Պատկերը ստացված է։ Այս ֆունկցիան հեռախոսի տեսախցիկով նկարում է տուփի շտրիխկոդը։"
        )
    ...
    if st.button("💾 ՊԱՀՊԱՆԵԼ ԲՈԼՈՐԸ ԲԱԶԱՅՈՒՄ", type="primary"):
        if model and current_imeis:
            imei_list = [
                i.strip() for i in current_imeis.split("\n") if i.strip()
            ]
            if not imei_list:
                ...
                for imei in imei_list:
                    check_url = (
                        f"{SUPABASE_URL}/rest/v1/{TABLE_NAME}?imei=eq.{imei}"
                    )
                    check_response = requests.get(check_url, headers=HEADERS)
                    if (
                        check_response.status_code == 200
                        and len(check_response.json()) > 0
                    ):
                ...
                if success_count > 0:
                    st.success(
                        f"🎉 Հաջողությամբ ավելացավ {success_count} հեռախոս։"
                    )
                    st.balloons()
                if skipped_imeis:
                    st.warning(
                        f"⚠️ Հետևյալ IMEI-ները արդեն կային բազայում. {', '.join(skipped_imeis)}"
                    )
        else:
...
# --- 3. 📊 SIRUS CLOUD BAZA ---
    ...
    read_response = requests.get(
        f"{SUPABASE_URL}/rest/v1/{TABLE_NAME}?select=*&order=id.asc",
        headers=HEADERS,
    )
    ...
    if read_response.status_code == 200:
        ...
        if data:
            ...
            search_query = st.text_input(
                "🔍 Արագ փնտրում (Մոդել, IMEI, Գույն, Մատակարար)...",
            ...
            if not df_filtered.empty:
                ...
                cols_to_show = [
                    c for c in ordered_cols if c in df_filtered.columns
                ]
                ...
                df_clean = df_clean.rename(
                    columns={
                        k: v for k, v in rename_dict.items() if k in df_clean.columns
                    }
                )
